...
const HOTEL_DISPLAY: Record<string, string> = {
  'maison-blanche': 'Hôtel Maison Blanche · Coyah',
  rama: 'Hôtel Rama · Kissidougou',
          ...
          <div className="relative z-20 flex items-center justify-center gap-2 pt-6 pb-2">
            {filtered.map((_, idx) => (
              <button
                key={idx}
                onClick={() => goTo(idx)}
                className={`relative h-1 w-10 rounded-full overflow-hidden bg-[#0D3B3E]/20 ${
                  idx === currentIndex ? 'shadow-sm shadow-[#F9A03F]/50' : ''
                }`}
