...
function getSupabaseServiceConfig(): SupabaseServiceConfig {
  // NOTE : le client service-role n'a besoin QUE de l'URL + service role key.
  // On ne doit PAS dépendre de NEXT_PUBLIC_SUPABASE_ANON_KEY ici (bug historique) :
  // cela cassait tout appel service-role (ex: webhook de paiement) si la clé anon
  // publique n'était pas configurée, alors qu'elle n'est pas utilisée par ce client.
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL
