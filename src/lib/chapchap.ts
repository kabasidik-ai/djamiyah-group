...
export function ensureSameOrigin(request: Request, siteUrl: string): boolean {
  ...
  if (
    ...
    (!vercelDeployUrl || !referer.startsWith(vercelDeployUrl))
  )
    return false
...
export function withSecurityHeaders(response: NextResponse, siteUrl: string): NextResponse {
  ...
  response.headers.set(
    ...
    'Content-Type, X-CCP-Signature, CCP-Signature, X-Signature'
  )
...
export function secureJson(payload: unknown, siteUrl: string, init?: ResponseInit): NextResponse {
  // Un statut 204 (No Content) ne doit JAMAIS avoir de corps de réponse
  // (RFC 7231 §6.3.5) — NextResponse.json() plante sinon (500 silencieux).
  const status = init?.status
  if (status === 204 || status === 304) {
    const response = new NextResponse(null, init)
    return withSecurityHeaders(response, siteUrl)
  }
