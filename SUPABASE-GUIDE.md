...
| --------------- | ------- | ------------- | -------- |
| Chambre Confort | premium | 520 000 GNF   | 2 pers.  |
...
- **Idempotent** : Le script peut être rejoué sans risque. Si les chambres existent déjà, elles
  seront mises à jour (`ON CONFLICT DO UPDATE`).
- **Transactions** : Le schéma (tables, index, triggers, RLS) est dans une transaction
  `BEGIN/COMMIT`. Les insertions sont séparées.
- **Images** : Les chemins d'images pointent vers `/images/maison-blanche/` dans le dossier
  `public/` du projet Next.js.
